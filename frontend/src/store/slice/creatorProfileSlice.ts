...
export type CreatorProfileSlice = {
  ...
  createProfile: (token: string, data: CreatorProfileData) => Promise<CreatorProfile>;
  updateProfile: (token: string, creatorId: string, data: Partial<CreatorProfileData>) => Promise<CreatorProfile>;
...
> = (set, get) => ({
  ...
  updateProfile: async (token: string, creatorId: string, data: Partial<CreatorProfileData>) => {
    set({ profileLoading: true, profileError: null });
  ...
  completeOnboarding: async (token: string, creatorId: string) => {
    try {
      ...
      const message =
        typeof error === "object" && error !== null && "message" in error
          ? (error as { message?: string }).message || "Failed to complete onboarding"
